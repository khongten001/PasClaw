  ...
  try
    ...
    begin
      ...
      (* tool_choice mapping (Anthropic Messages API).
           ...
           "none"     -> type:none   must not call any tool; needed because
                                      omitting the field with a non-empty
                                      tools array still lets the model call
                                      them
         Empty string means "do not emit; let provider default apply". *)
